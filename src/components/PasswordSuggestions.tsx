...
export const PasswordSuggestions = ({
  ...
  return <div className="w-full max-w-2xl space-y-4">
      <div className="items-center gap-2 text-foreground flex flex-row">
        <Sparkles className="h-5 w-5 text-secondary" />
        <h3 className="text-lg font-semibold tracking-wide font-sans text-justify">
          Reinforced Versions
        </h3>
      </div>

      <div className="grid gap-3">
        {suggestions.map((suggestion, index) => <Card key={index} className="p-4 bg-card border-2 border-border hover:border-primary transition-all duration-300 group hover:glow-cyber">
            <div className="flex items-start justify-between gap-4">
              ...
              <Button size="icon" variant="ghost" onClick={() => copyToClipboard(suggestion.password, index)} className="shrink-0 hover:text-primary hover:bg-primary/10 transition-all duration-200">
